...
<?= $this->Form->create() ?>
    <fieldset>
        <legend><?= __('Configurez les paramètres') ?></legend>
        <hr>
        <?= $this->Form->input('Tmdb_api_key') ?>
        <hr>
...
<div class="well col-xs-5 col-xs-offset-1 text-justify">
  Vous pouvez générer une clé d'api sur <a href="http://themoviedb.org ">themoviedb.org</a> <br>(Mon compte > API)
</div>
<div class="well col-xs-5 col-xs-offset-1 text-justify ">
